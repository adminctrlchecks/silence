...
`main` is at commit `7118089` and is **live in production** on both `https://silence-web-53h7.vercel.app` (Vercel, auto-deploys `main`) and `https://silence.ctrlchecks.ai` (Hostinger VPS, deployed via `deploy/deploy.sh`, kept in sync as of this commit). It contains: all of Phase 0-2, Phase 3 except the favicon/OG-image task, and Phase 4's Google OAuth item (built as client-side Google Identity Services, not the redirect flow originally sketched — see the Phase 4 checklist entry below for why). Everything else in the Work Queue (§3) is genuinely not started. Start at Phase 3's remaining favicon/OG-image task, then continue through Phase 4's remaining items, then Phase 5 onward, in order.
